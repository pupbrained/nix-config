{
  ...
  in
    builtins.filter (t: t != null) (map parseLine lines);
  ...
  linux-asahi-pkg = {
    ...
    extraConfig = lib.fold (patch: ex:
      ex
      ++ (parseExtraConfig (patch.extraConfig or ""))) []
    _kernelPatches;
    ...
    in
      builtins.listToAttrs (map makePair configList);
